...
def main(scp_dict:Dict[str,str], output_dir:str):
    """
    Arguments:
        scp_dict: {"key1": scp_path, "key2": scp_path_2 }. key should be the unique id for the dataset. 
            Note that each scp_path's utt's key should be split by ('-'), and the first element is the unique spk id of the wav.
    """
    os.makedirs(output_dir, exist_ok=True)
    res = []
    res_spk_dict = {}
    for _name, _path in scp_dict.items():
